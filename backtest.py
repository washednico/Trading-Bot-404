    ...
    for i in range(filter, len(historical_data)):
        ...
        if len(limit_orders) == 0:
            ...
            if indicators_sum >= minimum_indicators_to_open:
                ...
                position = execute_trade(order_type, config, current_data['close'])
                ...
                retracements  = get_fibonacci_levels(current_data, config, fill_price=current_data['close'])
        ...
        #Start check if condition is met
            for fibo_trades in limit_orders["fibo_orders"]:   
                ...
                if fibo_trades["type"] == "BUY":
                    if fibo_trades["price"] >= current_data["close"]: 
            ...
            if limit_orders["tp_orders"]["type"] == "BUY":
                if limit_orders["tp_orders"]["price"] >= current_data["close"]:
